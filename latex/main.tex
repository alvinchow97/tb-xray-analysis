% IEEEtran port of PAPER_FINAL.md (generated 2026-08-15).
...
%
% Remaining author actions before submission (tracked in PAPER_FINAL.md):
%  - Re-read refs [4] and [5] to confirm the Table VI characterisation wording.
...
\centering
\footnotesize
\begin{tabular}{p{3.6cm}p{4.6cm}p{4.4cm}p{3.4cm}}
\toprule
Study & Approach & Reported AUC & Cross-source external test? \\
\midrule
Lakhani \& Sundaram 2017~\cite{lakhani2017deep} & AlexNet/GoogLeNet &
0.99 (in-domain) & Not reported as held-out external \\
Rahman \emph{et al.} 2020~\cite{rahman2020reliable} &
CNNs $+$ segmentation (same Kaggle DB) & ${\sim}0.99$ & No (same source) \\
Sathitratanacheewin \emph{et al.} 2020~\cite{sathitratanacheewin2020deep} &
DCNN (Shenzhen-trained) & 0.85 in-domain / 0.71 external &
Yes (one cohort, NIH CXR8) \\
Karki \emph{et al.} 2022~\cite{karki2022generalization} &
CNNs, multi-task (DR-TB) & ${\sim}0.85$ in-domain / 0.65 external & Yes \\
Han \emph{et al.} 2025~\cite{han2025systematic} (meta, commercial) & Various &
0.85--0.97 (external) & Yes \\
\textbf{This work} & MobileNetV2, DenseNet-121, EfficientNet-B0 &
0.999 in-domain / 0.50--0.78 external$^{\dagger}$ &
\textbf{Yes (two NLM cohorts)} \\
\bottomrule
\multicolumn{4}{l}{\footnotesize $^{\dagger}$Backbone- and cohort-dependent.}
\end{tabular}
\end{table*}
